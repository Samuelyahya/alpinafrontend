...
import { StoreOptionSkeleton } from '@/src/components/skeleton';
      ...
      <div id='pickup-tab' className='tabcontent flex flex-col gap-4'>
        {isLoading
          ...
          : optionStores.map((store) => (
            <label
              key={store.id}
              className={`relative group w-full rounded-[20px] p-[10px_14px] bg-white transition-all duration-300 hover:ring-1 hover:ring-alpina-blue ${Number(selectedStore) === store.id
              ...
              <div className='flex items-center gap-[14px]'>
                ...
                <div className='w-6 h-6 flex shrink-0'>
                  <Image
                    src={TickCircle}
                    className={`w-6 h-6 ${Number(selectedStore) === store.id ? 'flex' : 'hidden'
                      }`}
                    alt='selected'
                  />
                </div>
